...
public class BookingCommandServiceImplTest {
    ...
    void arrange_sharedFixtures() {
        ...
        pendingBooking = new Booking(
                ...
                totalPrice, 2, BookingStatus.PENDING, Instant.now()
        );
    }
    ...
    class CreateBookingTests {
        ...
        void handle_validCommand_savesBookingAndDecrementsStock() {
            ...
            CreateBookingCommand command =
                    new CreateBookingCommand(1L, 10L, 1L, 2, Instant.now());
        ...
        void handle_validCommand_computesTotalPriceCorrectly() {
            // Arrange
            CreateBookingCommand command =
                    new CreateBookingCommand(1L, 10L, 1L, 3, Instant.now());
        ...
        @Test
        @DisplayName("handle(CreateBookingCommand) throws IllegalArgumentException when availability not found")
        void handle_availabilityNotFound_throwsIllegalArgument() {
            // Arrange
            CreateBookingCommand command =
                    new CreateBookingCommand(1L, 99L, 1L, 2, Instant.now());
        ...
        @Test
        @DisplayName("handle(CreateBookingCommand) throws IllegalStateException when stock is insufficient")
        void handle_insufficientStock_throwsIllegalState() {
            // Arrange
            CreateBookingCommand command =
                    new CreateBookingCommand(1L, 10L, 1L, 100, Instant.now());
    ...
    class InitiatePaymentTests {
        ...
        @Test
        @DisplayName("handle(InitiatePaymentCommand) throws IllegalArgumentException when booking not found")
    ...
    class ProcessPaymentTests {
        ...
        void handle_pendingBooking_bookingMarkedSucceeded() {
            ...
            ProcessPaymentCommand command = new ProcessPaymentCommand(
                    1L, "card", new TransactionId("pi_test_001")
            );
            when(bookingRepository.findById(1L)).thenReturn(Optional.of(pendingBooking));
            when(bookingRepository.save(pendingBooking)).thenReturn(pendingBooking);
        ...
        void handle_bookingNotFound_throwsIllegalArgument() {
            // Arrange
            ProcessPaymentCommand command = new ProcessPaymentCommand(
                    999L, "card", new TransactionId("pi_test_001")
            );
            when(bookingRepository.findById(999L)).thenReturn(Optional.empty());
    ...
    class FailPaymentTests {
        ...
        @Test
        @DisplayName("handle(FailPaymentCommand) throws IllegalArgumentException when booking not found")
        void handle_bookingNotFound_throwsIllegalArgument() {
            // Arrange
    ...
    class InitiateRefundTests {
        ...
        void handle_refundAlreadyExists_throwsIllegalState() {
            // Arrange
            pendingBooking.markAsSucceeded();
            pendingBooking.addRefund(new Refund(
                    totalPrice, "First refund", RefundStatus.PENDING, Instant.now()
            ));
        ...
        @Test
        @DisplayName("handle(InitiateRefundCommand) throws IllegalArgumentException when booking not found")
        void handle_bookingNotFound_throwsIllegalArgument() {
            // Arrange
