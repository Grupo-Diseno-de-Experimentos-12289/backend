...
public class CreateExperienceMediaCommandFromResourceAssemblerTest {

    @Test
    @DisplayName("toCommand should map CreateExperienceMediaResource to CreateExperienceMediaCommand (AAA)")
    void toCommandFromResource_ShouldMap() {
        //Arrange
        Experience exp = mock(Experience.class);
        when(exp.getId()).thenReturn(1L);
        var resource = new CreateExperienceMediaResource(1L, "http://example.com/media.jpg", "image/jpeg");
        //Act

        CreateExperienceMediaCommand expm = CreateExperienceMediaCommandFromResourceAssembler.toCommandFromResource(resource, exp);
