...
class ExperienceMediaControllerTest {
    ...
    @Mock
    private ExperienceMediaCommandService commandService;

    @Mock
    private ExperienceMediaQueryService queryService;
    ...
    void testCreate_Ok() {
        ...
        CreateExperienceMediaResource resource = new CreateExperienceMediaResource(1L,"url", "caption");
    ...
    void testCreate_ExperienceNotFound() {
        // Arrange
        Long experienceId = 1L;
        CreateExperienceMediaResource resource = new CreateExperienceMediaResource(2L,"url", "caption");

        when(experienceRepository.findById(experienceId)).thenReturn(Optional.empty());

        // Act & Assert
        assertThrows(RuntimeException.class, () -> experienceMediaController.create(experienceId, resource));
    }
    ...
    void testUpdate_Ok() {
        // Arrange
        Long mediaId = 100L;
        UpdateExperienceMediaResource resource = new UpdateExperienceMediaResource("new-url", "new-caption");

        when(commandService.handle(any(UpdateExperienceMediaCommand.class))).thenReturn(Optional.of(experienceMedia));

        // Act
        ResponseEntity<ExperienceMediaResource> response = experienceMediaController.update(mediaId, resource);
    ...
    void testUpdate_NotFound() {
        ...
        when(commandService.handle(any(UpdateExperienceMediaCommand.class))).thenReturn(Optional.empty());
    ...
    void testGetByExperience_Ok() {
        ...
        // Act
        ResponseEntity<List<ExperienceMediaResource>> response = experienceMediaController.getByExperience(experienceId);
