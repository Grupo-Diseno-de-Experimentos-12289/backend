...
public class CreateAvailabilityCommandFromResourceAssemblerTest {

    @Test
    @DisplayName("toCommandFromResource should map CreateAvailabilityResource to CreateAvailabilityCommand (AAA)")
    void toCommandFromResource_ShouldMap() {
        //Arrange
        var resource = new CreateAvailabilityResource(1L, LocalDateTime.of(2024,7,15,12,31,32), LocalDateTime.of(2025,7,15,12,31,32),20);
        Experience exp = mock(Experience.class);
        when(exp.getId()).thenReturn(1L);
        //Act
        CreateAvailabilityCommand cmd = CreateAvailabilityCommandFromResourceAssembler.toCommandFromResource(resource,exp);
