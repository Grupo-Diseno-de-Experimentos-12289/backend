...
class ExternalIamServiceTest {
    ...
    void testCreateUser_Success() {
        ...
        when(iamContextFacade.createUser(email, password, firstName, lastName, phone, roleNames)).thenReturn(expectedUserId);

        // Act
        Optional<UserId> result = externalIamService.createUser(email, password, firstName, lastName, phone, roleNames);

        // Assert
        assertTrue(result.isPresent());
        assertEquals(expectedUserId, result.get().userId());
        verify(iamContextFacade, times(1)).createUser(email, password, firstName, lastName, phone, roleNames);
    }
    ...
    void testCreateUser_Failure() {
        ...
        when(iamContextFacade.createUser(email, password, firstName, lastName, phone, roleNames)).thenReturn(0L);
