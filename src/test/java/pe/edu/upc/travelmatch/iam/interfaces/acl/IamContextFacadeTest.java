...
/**
 * Test class for {@link IamContextFacade}.
 * Validates the anti-corruption layer that exposes IAM capabilities to other bounded contexts.
 */
...
class IamContextFacadeTest {
    ...
    void createUser_ShouldReturnZero_WhenEmpty() {
        ...
        // Act
        Long result = iamContextFacade.createUser("user@test.com", "pass", "John", "Doe", "999", List.of("ROLE_TOURIST"));
    ...
    void fetchUserIdByEmail_ShouldReturnZero_WhenNotFound() {
        // Arrange
        when(userQueryService.handle(new GetUserByEmailQuery("notfound@test.com"))).thenReturn(Optional.empty());
    ...
    void fetchUserIdByEmail_ShouldReturnId_WhenFound() {
        ...
        when(userQueryService.handle(new GetUserByEmailQuery("found@test.com"))).thenReturn(Optional.of(user));
    ...
    void existsUserByEmailAndIdIsNot_ShouldReturnFalse_WhenNotFound() {
        // Arrange
        when(userQueryService.handle(new GetUserByEmailQuery("x@test.com"))).thenReturn(Optional.empty());
    ...
    void existsUserByEmailAndIdIsNot_ShouldReturnFalse_WhenSameId() {
        ...
        when(userQueryService.handle(new GetUserByEmailQuery("same@test.com"))).thenReturn(Optional.of(user));
    ...
    void existsUserByEmailAndIdIsNot_ShouldReturnTrue_WhenDifferentId() {
        ...
        when(userQueryService.handle(new GetUserByEmailQuery("diff@test.com"))).thenReturn(Optional.of(user));
