...
public class ExperienceMediaController {
  ...
  /** Constructs a new ExperienceMediaController. */
  ...
  public ResponseEntity<Long> create(
      @PathVariable Long experienceId, @RequestBody CreateExperienceMediaResource resource) {
    var experience =
    ...
    var command =
        CreateExperienceMediaCommandFromResourceAssembler.toCommandFromResource(
            resource, experience);
  ...
  @PutMapping("/{id}")
  public ResponseEntity<ExperienceMediaResource> update(
      @PathVariable Long id, @RequestBody UpdateExperienceMediaResource resource) {
  ...
  @Operation(
      ...
      responses = {@ApiResponse(responseCode = "200", description = "List of media returned")})
  @GetMapping
  public ResponseEntity<List<ExperienceMediaResource>> getAll() {
    var result =
        queryService.getAll().stream()
            .map(ExperienceMediaResourceFromEntityAssembler::toResourceFromEntity)
            .toList();
  ...
  @Operation(
      ...
      responses = {@ApiResponse(responseCode = "200", description = "Media list returned")})
  @GetMapping("/experience/{experienceId}")
  public ResponseEntity<List<ExperienceMediaResource>> getByExperience(
      @PathVariable Long experienceId) {
    var result =
        queryService.findByExperienceId(experienceId).stream()
            .map(ExperienceMediaResourceFromEntityAssembler::toResourceFromEntity)
            .toList();
    return ResponseEntity.ok(result);
  }
  ...
  @Operation(
      ...
      responses = {@ApiResponse(responseCode = "204", description = "Media deleted successfully")})
  @DeleteMapping("/{id}")
