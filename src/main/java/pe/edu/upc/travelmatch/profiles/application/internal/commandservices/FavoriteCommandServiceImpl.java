...
/** FavoriteCommandServiceImpl type. */
@Service
public class FavoriteCommandServiceImpl implements FavoriteCommandService {
  ...
  /** Constructs a new FavoriteCommandServiceImpl. */
  ...
  public Long handle(CreateFavoriteCommand command) {
    ...
    if (!externalExperiencesService.existsExperienceById(command.experienceId())) {
      throw new IllegalArgumentException(
          "Experience with id " + command.experienceId().experienceId() + " not found");
    }
  ...
  public void handle(DeleteFavoriteCommand command) {
    var existingFavorite =
        favoriteRepository.findByUserIdAndExperienceId(command.userId(), command.experienceId());
    if (existingFavorite.isEmpty()) {
      throw new IllegalArgumentException(
          "Favorite not found for user with id "
              + command.userId().userId()
              + " and experience with id "
              + command.experienceId().experienceId());
    }
