...
public class AgencyCommandServiceImpl implements AgencyCommandService {
  ...
  /** Constructs a new AgencyCommandServiceImpl. */
  public AgencyCommandServiceImpl(
      AgencyRepository agencyRepository, AgenciesContextFacade agenciesContextFacade) {
  ...
  public Long handle(CreateAgencyCommand command) {
    ...
    if (!agenciesContextFacade.existsUserById(command.userId())) {
      throw new IllegalArgumentException(
          "User with ID " + command.userId() + " does not exist in IAM.");
    }
    if (agencyRepository.existsByUserId(command.userId())) {
      throw new IllegalArgumentException(
          "Agency already exists for user ID " + command.userId() + ".");
    }
    ...
    if (agencyRepository.existsByContactEmail(command.contactEmail())) {
      throw new IllegalArgumentException(
          "Agency with contact email " + command.contactEmail() + " already exists.");
    }
  ...
  public Agency handle(UpdateAgencyCommand command) {
    ...
    if (command.contactEmail() != null
        && !command.contactEmail().equals(agencyToUpdate.getContactEmail())
        && agencyRepository.existsByContactEmailAndIdIsNot(
            command.contactEmail(), command.agencyId())) {
      throw new IllegalArgumentException(
          "Another agency with contact email " + command.contactEmail() + " already exists.");
    }

    AgencyName updatedName = command.name() != null ? new AgencyName(command.name()) : null;
    agencyToUpdate.updateDetails(
        updatedName, command.description(), command.contactEmail(), command.contactPhone());
